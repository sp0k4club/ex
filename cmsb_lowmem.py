# -*- coding: utf-8 -*-
# Python 2.7 - bounded-memory CMS fingerprinting batch runner
...
def detect_cms(url):
    ...
    try:
        ...
        html = response.raw.read(MAX_BODY_BYTES).lower()
...
def main():
    ...
    safe_print("[*] Streaming input: %d batches x %d threads = %d workers" % (batch_count, threads, batch_count * threads))
    safe_print("[*] Each response body is capped at 256 KB; input is never loaded fully into RAM.")
...
if __name__ == "__main__":
    try:
        ...
        safe_print("[!] %s" % error)
        safe_print("Usage: python2 cmsb_lowmem.py list.txt THREADS_PER_BATCH BATCHES [--verbose]")
